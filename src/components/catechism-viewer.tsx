          ...
          <div key={entry.id} className="group">
            <div className="flex flex-col space-y-4">
              <div className="relative">
                ...
                <h3 className={cn(
                  "telugu-text font-semibold border-l-4 border-accent pl-4",
                  isReadingMode ? "text-3xl md:text-4xl" : "text-2xl md:text-3xl"
                )}>
              ...
              <div className={cn(
                ...
                <span className="telugu-heading font-bold block mb-2 opacity-80">జవాబు:</span>
                {entry.answer}
              </div>

              {entry.explanation && (
                <div className={cn(
                  "mt-4 p-4 bg-muted/50 border-l-2 border-primary/20 rounded-r-lg italic telugu-text ml-5 text-foreground/70",
              ...
              {entry.scriptureReferences && entry.scriptureReferences.length > 0 && (
                <div className={cn(
                  ...
                  <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-3">లేఖన ఆధారాలు (Scripture Footnotes)</h4>
                  <ul className="space-y-3">
                    {entry.scriptureReferences.map((ref) => (
                      <li key={ref.id} className="text-base md:text-lg text-muted-foreground telugu-text flex gap-2">
